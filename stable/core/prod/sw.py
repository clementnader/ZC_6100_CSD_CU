...
class Sw(object):
    ...
    def check_const(self):
        ...
        for str_const_id in self.d_const:
            ...
            print('checking constraint: {0}, type: {1}'.format(str_const_id, str_type))
        ...
        for str_file_name in d_constr_map_lim_sect['files']:
            ...
            for str_sect_name, d_lim in d_constr_map_lim_sect['sect'].items():
                ...
                if 'start' in d_lim:
                    ...
                    i_start_addr, i_size_align = self._get_map_symb_addr_val_size_align(str_file_name, str_symb_sect, str_symb_name)
                    ...
                    if 'symb_off_sect' in d_lim['start'] and 'symb_off_name' in d_lim['start']:
                        ...
                        i_start_addr_off, i_size_align = self._get_map_symb_addr_val_size_align(str_file_name, str_symb_sect, str_symb_name)
                ...
                if 'end' in d_lim:
                    ...
                    i_end_addr, i_size_align = self._get_map_symb_addr_val_size_align(str_file_name, str_symb_sect, str_symb_name)
                    ...
                    if 'symb_off_sect' in d_lim['end'] and 'symb_off_name' in d_lim['end']:
                        ...
                        i_end_addr_off, i_size_align = self._get_map_symb_addr_val_size_align(str_file_name, str_symb_sect, str_symb_name)
                ...
                if b_start:
                    ...
                    for d_symb in self.d_map[str_file_name][str_sect_name][str_symb_low_addr]:
                        l_symb_names.append(d_symb['name'])
                    d_result_map_lim_sect[str_file_name][str_sect_name]['start'] = {'addr': '0x%08x' % i_low_addr, 'symb': l_symb_names}
                else:
                ...
                if b_end:
                    ...
                    for d_symb in self.d_map[str_file_name][str_sect_name][str_symb_high_addr]:
                        l_symb_names.append(d_symb['name'])
                    d_result_map_lim_sect[str_file_name][str_sect_name]['end'] = {'addr': '0x%08x' % i_high_addr, 'symb': l_symb_names}
                else:
                ...
                for str_symb_addr, l_symb in self.d_map[str_file_name][str_sect_name].items():
                    for d_symb in l_symb:
                        if d_symb['name'] in l_symb_names:
                            d_result_map_occur_symb[str_file_name][str_sect_name][d_symb['name']] = {'addr': '0x' + str_symb_addr, 'size': '0x' + d_symb['size_align']}
                ...
                for str_instr_addr_off, d_const_instr in d_symb_name_addr_off_const_instr[str_symb_name].items():
                    ...
                    if str_instr_addr in self.d_instr[str_file_name][str_symb_name]['instructions']:
                        ...
                        if str_mnem == d_instr['mnemonic']:
                            ...
                            d_result_instr_mand[str_file_name][str_symb_name]['instr'][str_instr_addr] = {'mnem': str_mnem, 'fields': {}}

                            if 'fields' in d_const_instr:
                                b_fields = True
                                for str_field_name in d_const_instr['fields']:
                                    d_result_instr_mand[str_file_name][str_symb_name]['instr'][str_instr_addr]['fields'][str_field_name] = d_instr['fields'][str_field_name]
                    ...
                    for str_instr_addr, d_instr in self.d_instr[str_file_name][str_symb_name]['instructions'].items():
                        ...
                        if b_instr_forb_occur:
                            if str_symb_name not in d_result_instr_forb_occur[str_file_name]:
                                str_symb_addr = self.d_instr[str_file_name][str_symb_name]['address']
                                i_symb_addr = int(str_symb_addr, 16)
                                d_result_instr_forb_occur[str_file_name][str_symb_name] = {'addr': "0x%08x" % i_symb_addr, 'instr': {}}

                            i_inst_addr = int(str_instr_addr,16)
                            str_instr_addr =  "0x%08x" % i_inst_addr
                            
                            if 'fields' in d_const_instr:
                                d_result_instr_forb_occur[str_file_name][str_symb_name]['instr'][str_instr_addr] = {'mnem': str_mnem, 'fields': d_const_instr['fields'] }
                            else:
                                d_result_instr_forb_occur[str_file_name][str_symb_name]['instr'][str_instr_addr] = {'mnem': str_mnem}
            ...
            for str_symb_name in self.d_instr[str_file_name]:
                for str_instr_addr, d_instr in self.d_instr[str_file_name][str_symb_name]['instructions'].items():
                    ...
                    if str_constr_instr_mnem1_incl == d_instr['mnemonic']:
                        ...
                        b_diff = True
                    elif d_instr['mnemonic'] in l_constr_instr_mnem1_incl or d_instr['mnemonic'] in l_constr_instr_mnem2_incl :
                        b_diff = False
                        ...
                        if str_symb_name not in d_result_instr_off[str_file_name]:
                            ...
                            d_result_instr_off[str_file_name][str_symb_name] = {'symb_addr': str_symb_addr, 'inst_addr': []}
                        ...
                        d_instr_addr_off = {'mnem1' : str_instr_addr_mnem1, 'mnem2': str_instr_addr_mnem2, 'off': str_instr_addr_off}
